...
| unit_id               | UUID      | FK(unit_master.id), NOT NULL | 単位ID（g／kg／mlなど）     |
| increment_amount      | integer   | NOT NULL                     | 増減量                |
| is_expiry_not_managed | boolean   | NOT NULL, DEFAULT false      | 賞味期限を管理しないか        |
...
| label        | text | NOT NULL       | 表示名（個／g／ml／本 など） |
| value        | text | NOT NULL, UNIQUE | 内部値（piece／g／ml など） |
...
| ingredient_id    | UUID      | FK(ingredients.id), NULLABLE    | 紐付く食材ID（過去登録済みの場合）         |
| custom_name      | text      | NULLABLE                        | 自由記述の食材名（ingredient_idがNULL時に使用） |
...
- ingredients：profiles.family_id = ingredients.family_id のみアクセス可
